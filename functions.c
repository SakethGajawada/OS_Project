...
User getUser(int user_id){
    ...
    int fd = open("Users.dat", O_RDONLY,0744);
    if(fd == -1) {
...
int ValidateAdminLogin(AdminCredentials *loginatt) {
    //int user_id = loginatt->id; 
    printf("inside the validateAdminLogin dbtion\n"); 
...
int db_delete_account(int account_no){
    ...
    int fd = open("Accounts.dat", O_RDWR,0744);
    if(fd == -1) {
      printf("Unable to open the file\n");
      exit(1);
    }
...
int db_modify_account_type(int account_no, int account_type){
    ...
    printf("in the db_modify_account_type dbtion\n");
    int fd = open("Users.dat", O_RDWR,0744);
    int result;
    if(fd == -1) {
      printf("Unable to open the file\n");
      exit(1);
...
int db_modify_admin_password(char *newpass, char *username){
    AdminCredentials validate; 
    printf("inside db_modify_admin_password dbtion\n");
    int fd = open("Admins.dat", O_RDWR,0744);
    if(fd == -1) {
      printf("Unable to open the file\n");
      exit(1);
    }
...
int db_add_account(int account_no){
    //this dbtion adds appends a new account to the accounts file.
    ...
    int fd = open("Accounts.dat", O_RDWR | O_APPEND);
    if(fd == -1) {
      printf("Unable to open the file\n");
      return -1; 
    }
